...
def test_packaged_manifest_fully_matches_runtime_descriptor():
    data = yaml.safe_load(files("forge_cli.adapters.codex").joinpath("resources", "adapter.yml").read_text(encoding="utf-8"))
    manifest = load_codex_adapter_descriptor().manifest
...
def test_packaged_evidence_fully_matches_runtime_descriptor():
    data = yaml.safe_load(files("forge_cli.adapters.codex").joinpath("resources", "capabilities.yml").read_text(encoding="utf-8"))
