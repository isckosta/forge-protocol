...
This skill registers a `PreToolUse` hook (active once this skill has been invoked in a session, not from session start) that denies: a `Bash` command matching an in-place-mutation shape (`sed -i`/`perl -i`/`truncate`/output redirection); and a direct `Edit` or `Write` call whose target file is one of `.forge/changes/*/manifest.yml`, `provenance.yml`, or `review.md` (CHG-0045). It does not match read-only or version-control commands (`cat`/`ls`/`git add`/`git commit`/`git status`/`git diff`/`git show`/`grep`) against the same paths. This remains a partial, illustrative guard, not a general security boundary (C-073): it does not see MCP filesystem tools, `NotebookEdit`, or a mutation of these paths issued by a subagent's own tool calls -- whether Claude Code applies this session's `PreToolUse` hooks to subagent-issued calls is unverified by this Adapter, and is not claimed here.
...
- Strict Review for this Flow is subject to the single "Reviewer/Resolver independence" section below; it is not restated per Flow.
