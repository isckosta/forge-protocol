...
def _instructions(protocol_id,flow_id,flow_content):
 ...
 if {"red_executed","red_failed_for_expected_reason"}.issubset(checks):lines.append("- Behavioral implementation requires valid RED.")
 ...
 if protocol_id>=2 and flow_id in{"fast","standard","full"}:
  lines += ["","### Reviewer/Resolver independence","","- Under Protocol 2, Strict Review must run in an Execution and Execution Context independent from the implementation or resolution that produced the revision under review.","- Merely changing Role inside the same conversation, thread, session, or reasoning context is self-review and cannot satisfy Strict Review.","- Finish the Implementation/Resolution and all reviewable evidence before freezing the review subject.","- Before freezing, ensure the effective reviewable Git workspace is clean: no committed post-subject delta, staged reviewable changes, unstaged reviewable changes, or Git-visible untracked reviewable files.","- Identify the concrete immutable subject revision. In Git, use the subject commit SHA; `revision.id` alone is not sufficient.","- Record the frozen subject in `provenance.yml`; the Review Iteration references it through `subject_provenance`.","- Only the exact Change-local `manifest.yml`, `provenance.yml`, and `review.md` paths are review-control metadata that may differ after the freeze; do not generalize that exception to the Change directory, matching basenames, symlinks, or rename targets.","- Git-ignored cache/editor/temp files do not count as reviewable workspace mutations for the freeze invariant.","- Re-check committed, staged, unstaged, and untracked reviewable deltas after recording review-control metadata.","- Start Strict Review against the frozen subject, not an ambiguous later HEAD or dirty checkout.","- Record the independent Reviewer execution through `reviewer_provenance`; it must bind to the exact same logical revision and immutable reference.","- Reviewer Execution and Context must both differ from the subject. Distinct invented IDs are not evidence.","- `claimed` is insufficient; `recorded` is repository-native self-recorded evidence and `verified` is stronger observer-backed evidence.","- After blocking findings are resolved, freeze the new Resolution revision and re-review that concrete revision independently."]
 return "\n".join(lines).rstrip()
