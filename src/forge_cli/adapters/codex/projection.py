...
def _instructions(protocol_id,flow_id,flow_content):
 ...
 if protocol_id>=2 and flow_id in{"fast","standard","full"}:
  lines += ["","### Reviewer/Resolver independence","","- Under Protocol 2, Strict Review must run in an Execution and Execution Context independent from the implementation or resolution that produced the revision under review.","- Merely changing Role inside the same conversation, thread, session, or reasoning context is self-review and cannot satisfy Strict Review.","- Finish the Implementation/Resolution and all reviewable evidence before freezing the review subject.","- Identify the concrete immutable subject revision. In Git, use the subject commit SHA; `revision.id` alone is not sufficient.","- Record the frozen subject in `provenance.yml`; the Review Iteration references it through `subject_provenance`.","- Review-control metadata (`manifest.yml`, `provenance.yml`, `review.md`) may follow the freeze; other subject changes require new provenance.","- Start Strict Review against the frozen subject, not an ambiguous later HEAD.","- Record the independent Reviewer execution through `reviewer_provenance`; it must bind to the exact same logical revision and immutable reference.","- Reviewer Execution and Context must both differ from the subject. Distinct invented IDs are not evidence.","- `claimed` is insufficient; `recorded` is repository-native self-recorded evidence and `verified` is stronger observer-backed evidence.","- After blocking findings are resolved, freeze the new Resolution revision and re-review that concrete revision independently."]
 return "\n".join(lines).rstrip()
