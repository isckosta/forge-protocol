...
## Addendum — Merge Readiness bookkeeping (post-Completion)

After Strict Review closed above and `manifest.yml` reached `state.current: complete`, `forge change merge-check` was run (not required for Completion itself, but relevant before opening a PR) and reported three gaps, none of them content defects in the reviewed subject: `MR-017` (`capabilities/README.md`/`capabilities/capability.md` had no materiality classification in `protocol/policies/merge-readiness.yml` — the same class of gap `CHG-0045`'s ADR-0018 records for its own new Adapter-generated paths); `MR-008` (`plan.md#approval-record`'s `content_digest` was computed before the R-008 fix and no longer matched the current, correctly-approved `plan.md`); `MR-006` (no `role: implementation` provenance record bound `verification.md`'s evidence to the final frozen review subject, `documentation-001`/`356f201`).

Fixing `MR-017` touches `protocol/policies/merge-readiness.yml` — outside this Change's own directory, and something `specification.md`/earlier Iterations' "Checked and found sound" sections correctly stated was unchanged *as of the commits they each reviewed*. This was presented to the user explicitly (in chat, with three options: fix all three including `protocol/`, fix only the non-`protocol/` bookkeeping, or leave `MERGE BLOCKED` for later); the user selected fixing all three. `capabilities/` was classified as a **material** prefix (not `permitted`, like `docs/`) — it is contract-defining canonical documentation analogous to `protocol/` itself, and future edits to it should go through the same Change governance rigor, not be freely editable like `docs/`.

`forge validate` did not require a further Strict Review Iteration for this fix: its own C-026 freeze-invariant check intentionally stops once `state.current == "complete"` (a documented, pre-existing carve-out, distinct from the Merge Readiness evaluation, which has no such carve-out per Protocol 2 §5/§14 — precisely why Merge Readiness caught what `forge validate` did not). Re-verified after the fix: `forge validate` → PASS; `forge change merge-check --base 7495615...39 --head c0a6e32...04` → **MERGE READY**; full suite → 750 passed, 2 warnings (unregressed — provenance/policy bookkeeping only, no code or test touched).
