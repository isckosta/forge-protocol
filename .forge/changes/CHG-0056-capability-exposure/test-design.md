...
- Unit: catalog ordering, invalid identity rejection, deterministic exposure, and Codex/Claude projection paths and references.
- Integration: packaged catalog loading, publication of all Capability skills, and automatic projection of a newly added catalog entry.
- Regression: existing workflow skill, Flow references, hooks, CLAUDE.md pointer, and ownership semantics remain unchanged.
- Negative boundary: no test introduces a Forge slash-command Protocol field or assumes Capability root cause or fix behavior.
