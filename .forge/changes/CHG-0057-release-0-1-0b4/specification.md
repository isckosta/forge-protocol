...
The package identifies CLI version `0.1.0b4` and the changelog contains a dated
matching section. No executable or Protocol behavior changes.

## Classification

This non-behavioral Change covers release metadata only.
...
#### Requirement
The version source is `0.1.0b4` and the changelog has a dated matching section.

#### Expected Behavior
The version is valid PEP 440 and does not alter Protocol versioning.
...
#### Acceptance
AC-001
Given the capability-exposure merge is on `main`
...
Complete: the metadata-only release boundary and expected result are explicit.
